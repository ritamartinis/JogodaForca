...
#define P 7 											// Nº de palavras no jogo
...
int converterMinusculaCorretor(char entrada[C], int tamanhoEntrada) //converte maiúsculas em minúsculas e retorna o valor 1 se algum carater fora fora de a-z E - (hifene)
{
	if (tamanhoEntrada == 1)
	{
		...
		for (int i = 0; i < tamanhoEntrada - 1; i++)				//tamanhoEntrada-1 porque é para verificar as letras nas posições da matriz de tamanho tamanhoEntrada
		{
			if (entrada[i] != '-')
			{
...
void game(char palavrasTodas[P][C], char palavraSel[C], char palavraMostrada[C])
{
	...
	do
	{
		...
		else
		{
			...
			//entrada[0] = tolower(entrada[0]);		// ERRO: SÓ CONVERTIA O 1º CARATER || tolower é uma função para passar maisculas para minusculas, caso o utilizador coloque uma maiscula
			...
			if (converterMinusculaCorretor(entrada, tamanhoEntrada) == 1)	// se o user introduzir um carater menor que a e maior que z
			{
				printf("\033[1;31m");
				printf("\n");
				printf("\t\t\t\tOs carateres permitidos são apenas de de A a Z sem espaços, e hifenes ao adivinhar palavras!");
				printf("\033[0m");
				Sleep(2000);
			}
			else
			{
				nTentativa += tentativasAnteriores(entrada, nTentativa, tentativasInseridas); //armazena as tentativas anteriores, caso jã não tenham sido inseridas
	...
	} while (vitoria == 0);		//não pode ser zero porque senão não mostra a ultima tentativa.
}
