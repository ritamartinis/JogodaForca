...
void menuInicial()
{
	...
	printf("\n");
	printf("\033[1;32m");
	escreverDevagar("\t\t\t\tEscolha uma opção: ");
	printf("\033[0m");
}
...
{
	return rand() % (P - 1); 								// Gera um número aleatório entre 0 e P, que é o número de palavras. Neste caso entre 0 e 6.
}
...
int contaLetrasRepetidas(char letraIns, char palavraSel[C])		//vai contar as letras que são repetidas
{
	int contador = 0;;											// var interna
	for (int i = 0; i < strlen(palavraSel); i++)
	{
		if (palavraSel[i] == letraIns)				// se a letra inserida for igual a uma letra já existente no vetor, soma +1. vai ficar 5 * o contador				
			contador += 1;
...
{
	char entrada[C];		// quando tenta uma palavra		// mas depois é atribuido ao letra inserida
	...
	do
	{
		...
		printf("\t\t\t\t|                                                 |\n");
		printf("\t\t\t\t|     Vidas restantes: %i       Palavra:%s     |\n", tentativasR, palavraMostrada);
		printf("\t\t\t\t|     Pontuação: %i                                |\n", pontos);
		printf("\t\t\t\t|                                                 |\n");
		printf("\t\t\t\t+=================================================+\n");
		printf("\033[0m");

		if (strcmp(palavraSel, palavraMostrada) == 0)			// se ganhar por letras
		{
			printf("\n");
			printf("\033[1;32m");
			printf("\t\t\t\t+=================================+\n");
			...
			vitoria = 1;
			Sleep(4000);
			break;		// sai do ciclo quando ganha por letras, senão continua:
		}
		else
		{
			printf("\n");
			...
			else									// se for zero, ficou sem vidas
			{
				...
				Sleep(4000);
				break;					// para sair do ciclo
			}

			entrada[0] = tolower(entrada[0]);		// tolower é uma função para passar maisculas para minusculas, caso o utilizador coloque uma maiscula

			if (entrada[0] < 'a' || entrada[0] > 'z')	// se o user introduzir um carater menor que a e maior que z
			{
				printf("\033[1;31m");
				printf("\n");
			...
			{
				if (strlen(entrada) == 1)                    // se for um carater, ou seja, uma letra
				{
					letraIns = entrada[0];
					if (compararLetra(letraIns, palavraSel, palavraMostrada) == 1)
					{
						...
						printf("\n");
						printf("\t\t\t\tLetra errada!\n");
						printf("\033[0m");						// instruções para mostrar a vermelho
						Sleep(2000);
					}
					else
					{
						...
						printf("\n");
						printf("\t\t\t\tLetra certa!\n");
						printf("\t\t\t\tGanhou mais %i pontos\n", contaLetrasRepetidas(letraIns, palavraSel) * 5);
						printf("\033[0m");						// instruções para aparecer a verde
						pontos += 5 * contaLetrasRepetidas(letraIns, palavraSel);							// pontos: 5 por cada carater certo!
						Sleep(2000);
					}
				}
				else
	...
	} while (tentativasR > -1 && vitoria == 0);		//não pode ser zero porque senão não mostra a ultima tentativa.
}
...
void main()
{
	...
	printf("\n");
	escreverDevagar("\t\t\t\t   Obrigado por jogares ");
	getch();
}
