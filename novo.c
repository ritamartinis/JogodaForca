...
void printfLetrasUsadas(int tentativasR, char letraIns)
{
	printf("Letras erradas já usadas: ");
	if (tentativasR == 0)
		printf(" ");
	for (int i = 0; i < tentativasR; i++)		//i var auxiliar do ciclo, criada aqui
		printf(" %c", letraIns);
}
...
void game(char palavrasTodas[P][C], char palavraSel[C], char palavraMostrada[C])
{
	...
	char palavraIns;		// palavra inserida pelo utilizador			// n está a ser usado nem no main nem aqui
	int tentativasR;	    // tentativas que o utilizador tem
	int idPalavra;		    // 
	int vitoria;			// var auxiliar de condição de paragem do ciclo
	//demasiadas var nao usadas e repetidas do main
			...
			else
			{
				if (strlen(entrada) == 1)                   // se for um carater, ou seja, uma letra
				{
					...
					if (compararLetra(letraIns, palavraSel, palavraMostrada) == 1)
					{
						...
						printf("\033[0m");
						Sleep(2000);
						printfLetrasUsadas(tentativasR, letraIns);
	...
	} while (tentativasR > -1 && vitoria == 0);		//não pode ser zero porque senão não mostra a ultima tentativa. aparece tentativa: 1. explicar-lhes a alteração
}
...
void main()
{
	...
	char letraIns;																							// N USADA NO MAIN
	char palavraIns;																						// N USADA EM LADO NENHUM
	int tentativasR;																						// N USADA NO MAIN
	int idPalavra;																							// indice aleatório das palavras, é o que vai sortear uma palavra
	int vitoria;																							// N USADA NO MAIN
	...
	do
	{
		...
		if (opt >= 0 && opt <= 4)
		{
			...
			case 3:
				...
				break;
			case 4:
				escreverDevagar("\t\t\t\tThis program was made by : kithar, ryuka, ritans\n");
