...
#define P 22 											// Nº de palavras no jogo
#define C 23 											// Nº de caracteres por palavra
...
void escreverDevagar(char text[])					// Função para escrever devagar. Recebe uma frase que é o char text[] - é um array. Criado aqui.
{
	int i = 0;										// Inicia o índice para percorrer a string
	while (text[i] != '\0')					    	// Continua até encontrar o caractere nulo
	{
		putchar(text[i]);                           // Imprime o caractere na posição atual. TEM QUE SER putchar e não printf! //é o putchar porque não tem vars, é só texto
...
{
	system("cls||clear");
																						// O caractere de escape \033 permite que os programas controlem a aparência do texto exibido no terminal. 
...
{
	printf("\n\t\t\t\tA carregar\033[1;32m opção %c \033[0m", opt);

	for (int i = 0; i < 16; i++)					    	// Continua até 16
	{
...
{
	return rand() % P; 									// Gera um número aleatório entre 0 inclusive e P, exclusive, que é o número de palavras. Neste caso entre 0 e 22.
}
...
void esconderPalavra(char palavraSel[C])					// Função para ocultar a palavra selecionada							
{
	...
	}														// a não ser que seja um hifen
}

int compararLetra(char entrada[C], char palavraSel[C], char palavraMostrada[C])  //Função que compara e verifica se o vetor entrada[c] corresponde a alguma das letras presentes na palavraSel
{
	...
	for (int i = 0; i < strlen(palavraSel); i++)
	{
		if (palavraSel[i] == entrada[0])					// Se um caractere em palavraSel corresponder ao entrada[0]
		{
			...
			errou = 0;										// Damos o output de 0 para a função game, o que indica que acertou a letra.
		}
	}
	return errou;		// Se não houver correspondência entre a entrada e palavraSel, o output enviado é de 1, o que indica que errou a letra.
}
...
int converterMinusculaVerificador(char entrada[C], int tamanhoEntrada) //converte maiúsculas em minúsculas e retorna o valor 1 se algum carater for fora de a-z ou - (hifen)
{
	for (int i = 0; i < tamanhoEntrada; i++)							//é para verificar as letras nas posições da matriz de tamanho tamanhoEntrada
	{
		...
		else if (tamanhoEntrada == 1)				// else, ou seja, se for apenas um hífen:
			return 1;					// serve para impedir que a tentativa do utilizador seja apenas um hífen. Dá erro. Mostra linha 335
	}
	return 0;
}

int tentativasAnteriores(char entrada[C], int nTentativa, char tentativasInseridas[T][C])	//Armazena as palavras/letras inseridas anteriormente SE não forem repetidas
{
	int naoRepete = 1;					// var aux  
										// 1== NUNCA foi usada, 0== JÁ foi usado 
	for (int i = 0; i < T; i++)
		if (strcmp(tentativasInseridas[i], entrada) == 0)		// Se (tentativasInseridas, entrada) forem iguais
			naoRepete = 0;										// Já foi usado, não acrescenta no ecrã às tentativas anteriores
	if (naoRepete == 1)											
		strcpy(tentativasInseridas[nTentativa], entrada);		// NUNCA foi usado, acrescenta no ecrã às tentativas anteriores
...
void jogo(char palavrasTodas[P][C], char palavraSel[C], char palavraMostrada[C])
{
	...
	system("cls||clear");
	idPalavra = sorteio();			 // Obtém um índice aleatório (retornado pela função sorteio) para selecionar uma palavra do conjunto	
	...
	esconderPalavra(palavraMostrada); // Esconde a palavra selecionada para que o jogador a descubra, usando a função esconderPalavra  
	tentativasR = 5;				// Define o número de tentativas disponíveis para o jogador					
	vitoria = 0;					// Inicializa a variável de vitória como falso (0) - 1 para vitória, 0 para derrota

	do                               // Ciclo principal do jogo 
	{
		ecraJogo(tentativasR, palavraMostrada, nTentativa, tentativasInseridas, pontos);	// Mostra o ecraJogo

		if (strcmp(palavraSel, palavraMostrada) == 0)	// Compara a palavraSel com a palavraMostrada, se for igual =0, é dada a VITÓRIA 
		{
			printf("\n\t\t\t\t+=================================================+\n");
			printf("\t\t\t\t|                    Parabéns!                    |\n");
			printf("\t\t\t\t|                 ");
		...
		else
		{
			...
			{
				printf("\n\t\t\t\tInsira um caractere de A a Z ou a palavra completa \n\n");	// Solicita uma entrada válida ao utilizador
				printf("\033[1;33m\t\t\t\t       Tentativa:\033[0m");
				scanf(" %22[^\n]s", entrada);		// Lê a entrada do usuário, permitindo até 22 caracteres +  \0 (colocado automaticamente pelo scanf)
				Beep(200, 200);
			...
			tamanhoEntrada = strlen(entrada);					// A varíavel tamanhoEntrada toma o valor do número de caracteres na string entrada até encontrar o caractere nulo \0
			if (converterMinusculaVerificador(entrada, tamanhoEntrada) == 1)	// se o user introduzir um caractere menor que a e maior que z
			{
				printf("\033[1;31m\n\t\t\t\tOs carateres permitidos são exclusivamente de A a Z.\n");
				printf("\t\t\t\tNão são permitidos espaços.\n");
				printf("\t\t\t\tQuando aplicável, deverá colocar o hífen.\033[0m");
				getch();
				Beep(200, 200);
			}
			else                                               // se o user introduzir um caractere/string CORRETO
			{
				...
				nTentativa += letraJaUtilizada;						//armazena as tentativas anteriores, caso não tenham sido inseridas anteriormente
				...
				if (tamanhoEntrada == 1)                    // se for um caractere, ou seja, uma LETRA
				{
					...
					{
						tentativasR = tentativasR - 1;                   //Decrementa o número de tentativas restantes quando a letra inserida é errada
						printf("\033[1;31m\n\t\t\t\tLetra errada!\033[0m\n");
					}
					else                                //  Se for uma LETRA CERTA
					{
						if (letraJaUtilizada == 1)		// se a letra NÃO foi usada, acontece isto:
						{
							...
							printf("\n\t\t\t\t\033[1;31m Letra certa mas já utilizada! \033[0m\n");
					}
					Sleep(2000);
				}
				else                   // Se for uma PALAVRA
				{
					...
					{
						strcpy(palavraMostrada, palavraSel);		// para atualizar os _ e mostrar a palavra completa, não censurada.
						...
						if (pontos == 0)							// se o user não tiver pontos da ronda dos carateres, começa as contas aqui.							
							...
							pontos += (strlen(palavraSel) - pontos / 5) * 15;		//pontos trazidos da ronda dos carateres, assim, nr de _ * 15, MENOS pontos da ronda dos carateres a dividir por 5 (que são pontos por cada letra)
					}					
	...
	} while (vitoria == 0);			// Condição de PARAGEM do Ciclo
}
...
void main()
{
	...
	srand(time(NULL));							// Necessário para complementar a função rand, contida no sorteio()


	do
	{
		menuInicial();
		opt = getch();							// Fica à espera de uma opção do utilizador
		Beep(200, 200);

		if (opt == '0' || opt == '1' || opt == '2' || opt == '3' || opt == '$')		// Só aceita ESTAS
		{
			switch (opt)
			{
			case '$':					// Função batota() escondida 
				system("cls||clear");
				idPalavra = sorteio();											// O valor de Id da palavra é dado pelo valor devolvido pela função sorteio
				strcpy(palavraSel, palavrasTodas[idPalavra]);                   // Copiar o idPalavra para a variável palavraSel                      
				strcpy(palavraMostrada, palavrasTodas[idPalavra]);              // Copiar o idPalavra para a variável palavraMostrada   
